...
class JobPostingController extends Controller
{
    ...
    //     $onlineJobs = $onlineJobs->paginate(3);
       
    //     $offlineJobs = $offlineJobs->paginate(3);
    ...
    public function store(Request $request)
    {
        ...
        // $location->desc = $request->details;
        // $location->save();
        // $locationId = 'LI' . now()->format('YmdHis');
        foreach ($request->province as $index => $provinceId) {
            if (!empty($provinceId) && !empty($request->district[$index])) {
                $location = new LocationWorking();
                // $location->location_code = $locationId;
                $location->province_id = $provinceId;
                $location->district_id = $request->district[$index];
                $location->desc = $request->detail ?? null;
                $location->save();
            }
        }
        ...
        $jobLocation->location_id = $location_id;
        $jobLocation->save();
        ...
        if ($request->status === 'draft') {
            return redirect()->route('jobs')->with('success', 'Job saved as draft.');
        }
        ...
        if ($jobLocation) {
            ...
            if ($locationDetails) {
                ...
                if (isset($request->detail)) {
                    $locationDetails->desc = $request->detail;
                }
