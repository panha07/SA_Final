...
@section('backend_content')
    ...
    <div class="container bg-white mb-4 p-4 rounded shadow-sm">
        ...
        <form action="{{ route('job') }}" method="GET" class=" d-flex col-lg-4 mb-3">
            <input type="text" class="form-control  me-2" name="search" placeholder="Please enter the job title" aria-label="Search" value="{{ request()->input('search') }}">
    ...
    <li class="nav-item">
        <a class="nav-link {{ request('status') === 'online' || !request('status') ? 'active' : '' }}" href="?status=online&search={{ request('search') }}">
            Online Posts ({{ $allOnlineJobs->count() }})
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ request('status') === 'offline' ? 'active' : '' }}" href="?status=offline&search={{ request('search') }}">
            Offline Posts ({{ $allOfflineJobs->count() }})
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ request('status') === 'draft' ? 'active' : '' }}" href="?status=draft&search={{ request('search') }}">
            Draft Posts
        </a>
    </li>
</ul>
...
{{--     
        <ul class="nav nav-tabs mb-4" id="postTabs" role="tablist">
            <li class="nav-item">
                <button class="nav-link active" id="online-posts-tab" data-bs-toggle="tab" data-bs-target="#online-posts" name="online"
                    type="button" role="tab">Online Posts ({{ $allOnlineJobs->count() }})</button>
            </li>
            <li class="nav-item">
                <button class="nav-link" id="offline-posts-tab" data-bs-toggle="tab" data-bs-target="#offline-posts" name="offline"
                    type="button" role="tab">Offline Posts ({{ $allOfflineJobs->count() }})</button>
            </li>
            <li class="nav-item">
        ...
        <div class="tab-content" id="postTabsContent">
            <div class="tab-pane fade {{ request('status', 'online') == 'online' ? 'show active' : '' }}" id="online-posts" role="tabpanel">
            ...
            <div class="tab-pane fade {{ request('status') == 'offline' ? 'show active' : '' }}" id="offline-posts" role="tabpanel">
                                ...
                                <div class="card-body">
                                    ...
                                    <div>
                                        ...
                                        <i class="bi bi-door-closed">Close</i>
                                            ...
                                            <form action="{{ route('jobs.copy', $job->id) }}" method="POST" id="copy-form-{{ $job->id }}">
                                                @csrf
